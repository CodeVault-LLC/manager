      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
        ...
        >
          {typeof system.battery === 'string' ? (
            ...
            <>
              <LabelValue label="Charge" value={`${system.battery.charge}%`} />
              <LabelValue label="Status" value={system.battery.status} />
            </>
          )}
        </InfoCard>

        <InfoCard title="Environment" icon={<FileClock className="w-5 h-5" />}>
          <LabelValue
            label="System Drive"
            value={system.environment?.system_drive}
          />
          <LabelValue label="OS Locale" value={system.environment?.os_locale} />
          <LabelValue
            label="User SID"
        ...
        <InfoCard title="Disks" icon={<HardDrive className="w-5 h-5" />}>
          {(system.disk_usage || []).map((disk: any) => (
            <div key={disk.drive} className="mb-3">
              ...
              <LabelValue label="Free Space" value={`${disk.free_percent}%`} />
            </div>
          ))}
        </InfoCard>
        ...
        <InfoCard title="Startup Programs" icon={<Info className="w-5 h-5" />}>
